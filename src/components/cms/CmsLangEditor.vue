  ...
  <div class="cms-lang-editor">
    ...
    >
      <div>
        ...
        <p style="color: #8892b0; font-size: 0.88rem">
          Inspect and update dictionary keys across all 12 supported languages.
        </p>
      </div>
      <div style="display: flex; gap: 10px">
        <button class="cms-btn cms-btn--secondary" @click="addKeyPrompt">
          + Add Translation Key
        </button>
    ...
    <div v-if="filteredEntries.length" class="cms-card">
      <h3 style="color: #66fcf1; font-size: 1.1rem; font-weight: 600; margin-bottom: 1rem">
        [{{ selectedLang.toUpperCase() }}] - {{ selectedCategory }} ({{ filteredEntries.length }}
        keys)
      </h3>
      ...
      <div style="display: flex; flex-direction: column; gap: 1rem">
        ...
        >
          <div
            ...
            "
          >
            <span style="color: #8892b0; font-family: monospace; font-size: 0.85rem">
              {{ entry.keyPath }}
            </span>
            <button
              class="cms-btn cms-btn--danger"
              style="padding: 2px 6px; font-size: 0.75rem"
              @click="deleteKey(entry.keyPath)"
            >
              ✕ Delete
            </button>
          </div>
  ...
  methods: {
    ...
    async saveCategoryData() {
      ...
      try {
        ...
        this.$emit(
          'notify',
          `Category [${this.selectedCategory}] saved for [${this.selectedLang.toUpperCase()}]!`
        )
      } catch (err) {
