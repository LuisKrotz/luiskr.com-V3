...
describe('Web Components Suite - Structure, Events & Reactive Fidelity (50+ Tests)', () => {
  ...
  describe('1. AppShell & Root Container', () => {
    test('AppRoot renders shell with progress bar, nav, modals, cookie banner and view outlet', () => {
      ...
      expect(shadow.querySelector('.progress-bar')).not.toBeNull()
      expect(shadow.querySelector('app-nav')).not.toBeNull()
      expect(shadow.querySelector('preferences-modal')).not.toBeNull()
      expect(shadow.querySelector('lang-dialog')).not.toBeNull()
      expect(shadow.querySelector('cookie-banner')).not.toBeNull()
      expect(shadow.querySelector('#view-outlet')).not.toBeNull()
    })
  ...
  describe('2. AppNav - Navigation Bar, Actions & States', () => {
    test('AppNav renders navigation links and action button with translations', () => {
      ...
      expect(shadow.querySelector('.nav-about-btn').textContent.trim()).toBe('About Me')
      expect(shadow.querySelector('.nav-action-btn').textContent.trim()).toBe('Contact Me')
    })

    test('updateScrollState transitions contact action button to "Scroll up"', () => {
      const nav = document.createElement('app-nav')
      document.body.appendChild(nav)
      nav.translations = {
        title: 'Luis',
      ...
      const updatedBtn = nav.shadowRoot.querySelector('.nav-action-btn')
      expect(updatedBtn.textContent.trim()).toBe('Scroll up')
      expect(updatedBtn.classList.contains('scroll-up')).toBe(true)

      nav.updateScrollState('contact', false)
      expect(nav.shadowRoot.querySelector('.nav-action-btn').textContent.trim()).toBe('Contact')
    })
    ...
    test('clicking logo navigates to home route', async () => {
      ...
      const nav = document.createElement('app-nav')
      document.body.appendChild(nav)
      const logoBtn = nav.shadowRoot.querySelector('.nav-logo-btn')
    ...
    test('clicking preferences button opens preferences modal in store', () => {
      const nav = document.createElement('app-nav')
      document.body.appendChild(nav)
      const prefBtn = nav.shadowRoot.querySelector('.nav-pref-btn')
  ...
  describe('3. HomeMosaic - Fibonacci Layout & Project Tiles', () => {
    test('HomeMosaic renders project items with responsive grid layout', () => {
      ...
      const cards = shadow.querySelectorAll('.home-mosaic-item')
      expect(cards.length).toBe(3)
      expect(cards[0].classList.contains('home-mosaic-item--featured')).toBe(true)
      expect(cards[1].classList.contains('home-mosaic-item--featured')).toBe(false)
    })
    ...
    test('HomeMosaic renders project titles and categories', () => {
      ...
      const shadow = mosaic.shadowRoot
      const titles = shadow.querySelectorAll('.home-mosaic-title')
    ...
    test('HomeMosaic empty state handles gracefully without throwing', () => {
      ...
      document.body.appendChild(mosaic)
      mosaic.processedItems = []
      expect(mosaic.shadowRoot.querySelectorAll('.home-mosaic-item').length).toBe(0)
    })
  })
  ...
  describe('4. AboutSection - Profile Gravatar & Localized Bio Columns', () => {
    test('renders Gravatar image with responsive density descriptors and alt text', () => {
      ...
      const img = shadow.querySelector('.about-profile-picture-img')
    ...
    test('renders DrawText components for title and bio columns', () => {
      ...
      const drawTexts = about.shadowRoot.querySelectorAll('draw-text')
  ...
  describe('5. ContactSection - Localized Social Channels & Mailto URI', () => {
    test('renders title and social media links with SVG icons', () => {
      ...
      const links = shadow.querySelectorAll('.contact-social-link')
  ...
  describe('6. PreferencesModal - Themes, Motion Toggles & Keyboard ESC', () => {
    test('modal is hidden until opened via store', () => {
      const modal = document.createElement('preferences-modal')
      document.body.appendChild(modal)
      expect(modal.shadowRoot.querySelector('.pref-dialog')).toBeNull()

      store.commit('togglePreferencesModal', true)
      expect(modal.shadowRoot.querySelector('.pref-dialog')).not.toBeNull()
    })
    ...
    test('clicking backdrop overlay closes modal', () => {
      ...
      const backdrop = modal.shadowRoot.querySelector('.pref-backdrop')
      if (backdrop) {
        backdrop.click()
        expect(store.getters.getPreferencesOpen()).toBe(false)
      }
  ...
  describe('8. MediaFigure & MediaModal - Responsive Images & Video Lightbox', () => {
    test('MediaFigure renders image with expand button when can-expand="true"', () => {
      const media = document.createElement('media-figure')
      ...
      expect(fig.classList.contains('internal-expand')).toBe(true)
      expect(shadow.querySelector('.expand-modal-open-1')).not.toBeNull()
    })
    ...
    test('clicking expand button opens MediaModal via store.commit("setModal")', () => {
      ...
      const expandBtn = media.shadowRoot.querySelector('.expand-modal-open-1')
      expandBtn.click()
    ...
    test('MediaExpanded renders media preview and content wrapper', () => {
      const modal = document.createElement('media-expanded')
      ...
      expect(shadow.querySelector('.expand-modal-content')).not.toBeNull()
    })
    ...
    test('startClose triggers modal exit and store reset', () => {
      ...
      expect(modal.shadowRoot.querySelector('.expand-modal-content').classList.contains('expand-modal--closing')).toBe(true)
    })
  })
})
